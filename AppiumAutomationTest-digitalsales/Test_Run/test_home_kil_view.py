...
from Home_kil.test_content_unit import test_home_content_unit
from Home_kil.test_home_etc import test_home_notice_count,test_home_notice_click
...
from Home_kil.test_item import _perform_single_item_test,test_checklist_41,test_checklist_42,test_checklist_43,test_checklist_44,test_checklist_45,test_checklist_46
...
def test_home_kil_view_run(flow_tester, sheets_service, tester_name):
    ...
    #
    #     content_unit_passed, content_unit_message = test_product_shortcut_to_water_purifier(flow_tester)
    #
    ...
    #
    #     content_unit_passed, content_unit_message = (flow_tester)
    #
    ...
    # test_no_counter = 37
    # try:
    #     # [Seller app checklist-21,22,23] --- 콘텐츠 유닛 기능 확인 테스트 실행 ---
    #     # 세 개의 체크리스트를 하나의 시나리오로 묶어 테스트합니다. 대표로 21번을 사용합니다.
    #     test_no = f"Seller app checklist-{test_no_counter}"
    #     print(f"\n--- {test_no}: 콘텐츠 유닛 기능 확인 (항목 수, 스와이프, 클릭) ---")
    #
    #     content_unit_passed, content_unit_message = test_home_content_unit(flow_tester)
    #
    ...
    # ---------- 독바 전체메뉴 test_checklist_43 확인 테스트 실행 ----------
    try:
        test_no_counter += 1
        test_no = f"Seller app checklist-{test_no_counter}"
        print(f"\n--- {test_no}:  독바 전체메뉴 노출 확인 ---")
    ...
    # --- 독바 전체메뉴 test_checklist_44 확인 테스트 실행 ---
    try:
        test_no_counter += 1
        test_no = f"Seller app checklist-{test_no_counter}"
        print(f"\n--- {test_no}:  독바 전체메뉴 노출 확인 ---")
    ...
    # --- 독바 전체메뉴 test_checklist_45 확인 테스트 실행 ---
    try:
        test_no_counter += 1
        test_no = f"Seller app checklist-{test_no_counter}"
        print(f"\n--- {test_no}:  독바 전체메뉴 노출 확인 ---")
    ...
    # --- 독바 전체메뉴 test_checklist_46 확인 테스트 실행 ---
    try:
        test_no_counter += 1
        test_no = f"Seller app checklist-{test_no_counter}"
        print(f"\n--- {test_no}:  독바 전체메뉴 노출 확인 ---")

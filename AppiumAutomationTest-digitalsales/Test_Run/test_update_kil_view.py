...
from Update_kil.test_app_permissions import (
    test_verify_permission_guide_title,
    test_verify_required_permissions,
    test_verify_optional_permissions_with_scroll,
    test_confirm_permissions_and_navigate_to_login,
    test_login_after_relaunch_and_verify_version,
    test_verify_no_permission_guide_after_relaunch
)
...
def test_update_kil_view_run(flow_tester, sheets_service, tester_name):
    """
    (리팩터링) 모든 앱 업데이트 테스트 시나리오를 리스트 기반으로 실행합니다.
    """
    print("=========================================")
    ...
    overall_test_message = "모든 앱 업데이트 확인 테스트 시나리오가 성공적으로 완료되었습니다."

    # --- 앱 업데이트/권한 테스트 케이스 정의 ---
    # (테스트명, 테스트 함수, Checklist ID)
    test_scenarios = [
        ("업데이트 얼럿 노출 확인", test_verify_update_alert, 1),
        ("업데이트 확인", test_perform_app_update, 2),
        ("재실행 업데이트 노출 확인", test_login_after_relaunch_and_verify_version, 3),
        ("접근권한 안내 팝업 노출 확인", test_verify_permission_guide_title, 4),
        ("재실행 접근권한 미노출 확인", test_verify_no_permission_guide_after_relaunch, 5),
        ("필수적 접근권한 확인", test_verify_required_permissions, 6),
        ("선택적 접근권한 확인 (스크롤 포함)", test_verify_optional_permissions_with_scroll, 7),
        ("접근권한 팝업 종료 확인", test_confirm_permissions_and_navigate_to_login, 8),
    ]
    ...
    for test_name, test_function, checklist_id in test_scenarios:
        ...
        try:
            ...
            if not test_passed:
                ...
                overall_test_message = "일부 앱 업데이트 테스트에서 실패가 발생했습니다."
            ...
            overall_test_passed = False
            overall_test_message = "일부 앱 업데이트 테스트에서 심각한 오류가 발생했습니다."
            status = "Fail"
